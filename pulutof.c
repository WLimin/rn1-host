...
tof3d_scan_t* get_tof3d() {
	...
	tof3d_rd++;
	if (tof3d_rd >= TOF3D_RING_BUF_LEN) {
		tof3d_rd = 0;
	}
	return ret;
}
...
pulutof_frame_t* get_pulutof_frame() {
	...
	pulutof_ringbuf_rd++;
	if (pulutof_ringbuf_rd >= PULUTOF_RINGBUF_LEN) {
		pulutof_ringbuf_rd = 0;
	}
	return ret;
}
					...
					if (z > 700 || (z > -180.0 && z < 130.0) || (n_valids > 7 && n_conforming > 5)) {
						...
						if (do_send_pointcloud == 1) { // relative to robot
							...
							if (tof3ds[tof3d_wr].n_points < 4 * TOF_XS * TOF_YS) {
								...
								float x_world = d * cos(ver_ang + sensor_yang) * cos(hor_ang + sensor_ang + robot_ang)
										+ sensor_x + in->robot_pos.x;
								float y_world = -1
										* (d * cos(ver_ang + sensor_yang) * sin(hor_ang + sensor_ang + robot_ang))
										+ sensor_y + in->robot_pos.y;
...
static void process_pulutof_frame(pulutof_frame_t* in) {
	...
	int expected_sidx = prev_sidx + 1;
	if (expected_sidx >= NUM_PULUTOFS) {
		expected_sidx = 0;
	}

	if (running_ok && expected_sidx != sidx) {
		printf(
				"WARNING:process_pulutof_frame: unexpected sensor idx %d, previous was %d, was expecting %d. Ignoring until 0\n",
...
//              process_objmap();
				tof3d_wr++;
				if (tof3d_wr >= TOF3D_RING_BUF_LEN) {
					tof3d_wr = 0;
				}
...
static void gen_ang_tables() {
	...
	for (int calyy = 0; calyy < GEOCAL_N_Y + 1; calyy++) {
		// Left half, extrapolate the first X record
		{
			...
			float cur_ax = -1 * lens_quadrant_coords[calyy][0].ang_x
					+ ax_per_px * lens_quadrant_coords[calyy][0].sens_x;
		...
		for (int calxx = 0; calxx < GEOCAL_N_X; calxx++) {
			...
			for (int pxx = lens_quadrant_coords[calyy][calxx].sens_x;
					pxx < lens_quadrant_coords[calyy][calxx + 1].sens_x; pxx++) {
				outp_ang(pxx, (int) (cur_py + 0.5), cur_ax, cur_ay);
				cur_py += py_per_px;
				cur_ax -= ax_per_px;
			}
		...
		outp_ang(lens_quadrant_coords[calyy][GEOCAL_N_X].sens_x, lens_quadrant_coords[calyy][GEOCAL_N_X].sens_y,
				lens_quadrant_coords[calyy][GEOCAL_N_X].ang_x, -1 * lens_quadrant_coords[calyy][GEOCAL_N_X].ang_y);
	...
	for (int pxx = 0; pxx < TOF_XS / 2 + 1; pxx++) {
		...
		for (int pyy = 0; pyy < TOF_YS / 2; pyy++) {
			if (x_angs[pyy * TOF_XS + pxx] != 999.0) {
				...
				// Find next:
					int next_py = pyy + 1;
					while (x_angs[next_py * TOF_XS + pxx] == 999.0) {
						next_py++;
					}
	...
	for (int pxx = 0; pxx < TOF_XS / 2 + 1; pxx++) {
		...
		x_angs[(TOF_YS - 1) * TOF_XS + pxx] = x_angs[(TOF_YS - 2) * TOF_XS + pxx]
				- (x_angs[(TOF_YS - 3) * TOF_XS + pxx] - x_angs[(TOF_YS - 2) * TOF_XS + pxx]);
		y_angs[(TOF_YS - 1) * TOF_XS + pxx] = y_angs[(TOF_YS - 2) * TOF_XS + pxx]
				- (y_angs[(TOF_YS - 3) * TOF_XS + pxx] - y_angs[(TOF_YS - 2) * TOF_XS + pxx]);
	}
...
static int read_frame() {
	...
	if (verbose_mode) {
		...
		for (int i = 1; i < 24; i++) {
			printf(">%d:%.1f ", i,
					(float) (pulutof_ringbuf[pulutof_ringbuf_wr].timestamps[i]
							- pulutof_ringbuf[pulutof_ringbuf_wr].timestamps[i - 1]) / 10.0);
		}
		printf("\n");
		printf("dbg_i32:\n");
		for (int i = 0; i < 8; i++) {
	...
	pulutof_ringbuf_wr++;
	if (pulutof_ringbuf_wr >= PULUTOF_RINGBUF_LEN) {
		pulutof_ringbuf_wr = 0;
	}
...
void* pulutof_poll_thread() {
	...
	init_spi();
	while (running) {
		int next = pulutof_ringbuf_wr + 1;
		if (next >= PULUTOF_RINGBUF_LEN) {
			next = 0;
		}
